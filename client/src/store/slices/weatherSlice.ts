...
export const fetchWeather = createAsyncThunk('/weather/fetchWeather', async () => {
	try {
		const response: AxiosResponse<IWeatherData> = await axios.get(
			`http://192.168.2.23:3000/weather/`
		);
...
const weatherSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(fetchWeather.fulfilled, (state, action: PayloadAction<IWeatherData>) => {
...
export const selectLocation = createSelector(
	(state: RootState) => state.weather.data.name,
	...
	(name, country, state, dt, timezone) => {
		return {
			name,
			country: country || '',
			state: state || '',
...
export const selectHourlyWeather = createSelector(
	...
	(hourly, timezone, userMetrics) => {
		...
		return {
			scale: hourlyService.getScale(),
			curve: hourlyService.drawCurve(),
			precipitationRects: hourlyService.getPrecipitationRects(),
			precipitationDesc: hourlyService.getPrecipitationDescription(),
			timeLine: hourlyService.getTimeLine(),
			wetherDesc: hourlyService.getWeatherDescription(),
		};
	}
);
...
export const selectWeatherIconId = createSelector(
	...
	(icon) => {
		const weatherId = icon;
		const weatherIdDigit = icon.slice(0, 2);
		const weatherIdLetter = weatherId.charAt(2);
...
export const selectPrecipitation = createSelector(
	...
	(precipitation, timezone) => {
		const gradientColors = getPrecipitationColors(
			Math.max(...precipitation.map((color) => color.precipitation))
		);

		const precipitationService = new PrecipitationService(precipitation, timezone);
		const main = precipitationService.drawCurve(true);
		const timeLine = precipitationService.getTimeLine();
		const axis = precipitationService.getAxis();

		return {
			gradientColors,
			mainCurve: main.mainCurve,
			backCurve: main.backPathCurve,
			timeLine,
			axis,
		};
	}
...
export const selectDaily = createSelector(
	...
	(daily, timezone, userMetric) => {
		...
		const colors = getTempritureScale(convertedMinTemp.value, convertedMaxTemp.value, userMetric);
...
export const selectPopupScale = (value: number) =>
	createSelector(
		...
		(dailyTemp, userMetrics) => {
			console.log('metric', userMetrics);
